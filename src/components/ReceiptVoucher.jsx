...
export const ReceiptVoucherPopup = ({ isOpen, onClose }) => {
  ...
  const saveVoucher = async (printAfterSave = false) => {
    ...
    try {
      ...
      const logEntry = {
        _id: uuidv4(),
        statement: `تم إنشاء سند قبض من ${recipient} بقيمة ${items[0].price} د.أ`,
        user_id: user_id,
      ...
      toast.error('خطأ في حفظ سند القبض');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 flex flex-col h-screen z-50">
      <div className="flex justify-between items-center p-4 border-b bg-white shadow-sm">
        <h1 className="text-xl font-semibold text-right text-blue-900">سند قبض</h1>
      ...
      <div className="p-6 flex-1 max-w-5xl mx-auto w-full overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <input type="date" className="w-full p-3 border border-gray-200 rounded-lg text-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" value={date} onChange={(e) => setDate(e.target.value)} />
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <label className="block text-sm font-medium mb-2 text-gray-600">اسم المسلّم</label>
            <input
              type="text"
                ...
                <tr>
                  ...
                  <td className="p-2">
                    <div className="flex items-center gap-1 justify-center">
                      <input type="number" className="w-24 p-2 rounded-lg bg-gray-50 text-center" value={items[0].price || ''} onChange={(e) => setItems([{ ...items[0], price: parseFloat(e.target.value) || 0 }])} min="0" />
        ...
        <div className="flex justify-between items-start bg-white p-6 rounded-lg shadow-sm">
          <div className="space-y-3 text-right">
            <p className="text-gray-600">المجموع الفرعي: {items[0].price} د.أ</p>
...
export default ReceiptVoucherPopup;
