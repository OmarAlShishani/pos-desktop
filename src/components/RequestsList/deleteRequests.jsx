...
export default function DeleteRequests({ requests, handleRequest }) {
  return (
      ...
      <tbody>
        {requests.map((request, index) => (
          <tr key={index} className="border-b text-right">
            ...
            <td className="p-3">{request.productNames}</td>
            <td className="p-3">
              <span
            ...
            <td className="px-6 py-4 whitespace-nowrap text-center text-sm">
              <button onClick={() => handleRequest(request, 'delete', 'approve')} className="text-green-600 hover:text-green-900 font-medium ml-4">
                موافقة
              </button>
              <button onClick={() => handleRequest(request, 'delete', 'decline')} className="text-red-600 hover:text-red-900 font-medium mr-4">
                رفض
              </button>
            </td>
          </tr>
