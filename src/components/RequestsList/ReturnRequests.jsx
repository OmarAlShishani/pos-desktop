...
export default function ReturnRequests({ requests, handleRequest }) {
  return (
    <table className="w-full border-collapse bg-white shadow-lg rounded-lg">
      <thead>
        <tr className="bg-blue-100 text-right">
          ...
          <th className="px-6 py-3 text-center text-sm font-medium text-[#303573] uppercase">المنتجات</th>
        ...
        {requests.map((request, index) => (
          <tr key={index} className="border-b text-center">
            ...
            <td className="p-3">
              <ul className="list-disc list-inside">
                {request.items.map((item) => (
                  <li>
                    {item.name_ar || item.name} (الكمية: {item.quantity})
                  </li>
                ))}
              </ul>
            </td>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-center text-sm">
              <button onClick={() => handleRequest(request, 'return', 'approve')} className="text-green-600 hover:text-green-900 font-medium ml-4">
                موافقة
              </button>
              <button onClick={() => handleRequest(request, 'return', 'decline')} className="text-red-600 hover:text-red-900 font-medium mr-4">
                رفض
              </button>
            </td>
          </tr>
